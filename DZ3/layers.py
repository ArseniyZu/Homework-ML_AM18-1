...
class ReLULayer:
    ...
    def backward(self, d_out, x_prev):
        """
        Backward pass
...
class FullyConnectedLayer:
    ...
    def forward(self, X):
        ...
        # raise Exception("Not implemented!")
        prev_X = X
        X_dot_w = X.dot(self.W.value)
        result = np.add(X_dot_w, self.B.value)
        return result, prev_X

    def backward(self, d_out, prev_x):
        """
        Backward pass
        ...
        self.B.grad += np.dot(np.ones((1, prev_x.shape[0])), d_out)
        self.W.grad += prev_x.T.dot(d_out)
